...
bool Console::promptConfirm(Style st, const std::string &question) {
    ...
    const std::string msg1 = _("'a' Yes - 'b' No");
    const std::string msg2 = _("'a' Confirm - 'b' Cancel");
    const std::string msg3 = _("'y' Confirm - 'b' Cancel");
    std::string msg;
    switch (st & 0x0F) {
        case ST_YES_NO:
            msg = msg1;
    ...
    if (!(st & ST_MULTILINE)) {
        ...
        Console::consolePrintPos(31, initialYPos + 2 + nLines, msg.c_str());
    }
    ...
    std::string read_key;
    std::cin >> read_key;
    for (auto &upper : read_key) upper = toupper(upper);
    if (st & ST_WIPE) {
        if (read_key == "Y") {
            ret = 1;
        }
...
void Console::consolePrintPosAligned([[maybe_unused]] int y, [[maybe_unused]] uint16_t offset, [[maybe_unused]] uint8_t align, const char *format, ...) {
    char *tmp = nullptr;
    //int x = 0;
    //y += Y_OFFSET;
...
void Console::consolePrintPos([[maybe_unused]] int x, [[maybe_unused]] int y, const char *format, ...) { // Source: ftpiiu
    char *tmp = nullptr;
    //y += Y_OFFSET;
...
void Console::kConsolePrintPos([[maybe_unused]] int x, [[maybe_unused]] int y, [[maybe_unused]] int x_offset, const char *format, ...) { // Source: ftpiiu
    char *tmp = nullptr;
    //y += Y_OFFSET;
